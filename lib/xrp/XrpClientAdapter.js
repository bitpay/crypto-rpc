...
class XrpClientAdapter extends xrpl.Client {
  ...
  async getTransactions(acceptanceAddress, {
    ...
    const filteredTransactions = result.transactions.filter(({ meta, tx }) => {
      ...
      return (
        TYPES.has(TransactionType.toLowerCase())) // filter on transaction type
        // INITIATED: If true, return only transactions initiated by the account specified by acceptanceAddress. If false, return only transactions NOT initiated by account specified by acceptanceAddress
        && (INITIATED === isTxInitiatedByAcceptanceAddress)
        // If EXCLUDE_FAILURES, return false if transaction result is not SUCCESS
        && (!EXCLUDE_FAILURES || meta.TransactionResult === SUPPORTED_TRANSACTION_RESULT_CODES.tes.SUCCESS);
    });
