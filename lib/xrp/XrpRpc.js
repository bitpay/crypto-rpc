...
class XrpRpc {
  ...
  async getAccountFlags({ address }) {
    ...
    const accountFlags = [];
    let flags = accountInfo.account_data.Flags;
    for (let flag of Object.keys(hexValues)) {
      if (Number(hexValues[flag] & flags) !== 0) {
        accountFlags.push(flag);
      }
    }
    accountFlags.push('lsfRequireDestTag');
    return accountFlags;
  }
}
