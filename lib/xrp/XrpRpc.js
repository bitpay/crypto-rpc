...
class XrpRpc {
  ...
  async sendToAddress({ address, amount, passphrase, tag, invoiceID, secret, alreadyConnected}) {
    let signedTx = await this.signTransaction({address, amount, passphrase, tag, invoiceID, secret, alreadyConnected});
    let txHash = await this.sendRawTransaction({ signedTx, alreadyConnected });
    return txHash;
  }
  ...
  async sendRawTransactionMany({ signedTxArray }) {
    let resultArray = [];
    for (const signedTx of signedTxArray) {
      const emitData = { signedTx };
      try {
        let txHash = await this.sendRawTransaction({ signedTx });
  ...
  async getRawTransaction({ txid }) {
    return (await this.asyncCall('tx', [{transaction:txid, binary:true}])).tx;
  }

  async sendRawTransaction({ signedTx, alreadyConnected }) {
    let response = await this.asyncCall('submit', [signedTx], alreadyConnected);
  ...
  async decodeRawTransaction({ rawTx }) {
    return rippleBinaryCodec.decode(rawTx);
  }
