...
class XrpRpc {
  ...
  async sendToAddress({ address, amount, passphrase, tag, invoiceID, secret, alreadyConnected}) {
    return this.signTransaction({address, amount, passphrase, tag, invoiceID, secret, alreadyConnected}).then((signedTx) => {
      return this.sendRawTransaction({ signedTx, alreadyConnected }).then((txHash) => {
        return txHash;
      });
    });
  }
