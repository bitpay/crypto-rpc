...
class XrpRpc {
  constructor(config) {
    ...
    this.rpc = { connectionString, timeout }; /* RIP-lo new RippleAPI({
  ...
  async decodeRawTransaction({ rawTx }) {
    let txid = JSON.stringify(rawTx); /* RIP-lo rippleHashes.computeBinaryTransactionHash(rawTx); */
    let txJSON = ''; /* RIP-lo rippleBinaryCodec.decode(rawTx); */
    txJSON.hash = txid;
    return txJSON;
  }
