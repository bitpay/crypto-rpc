...
const { getTransactionAdapter } = require('./adapters');
...
class XrpRpc {
  ...
  async getTransaction({ txid }) {
    try {
      ...
      return getTransactionAdapter(result);
    } catch (err) {
      if (err && err.data && err.data.error === 'txnNotFound') {
        return null;
      }
  ...
  async getTransactions({ address, options }) {
    try {
      // In xrpl v4, account_tx does not error on unfound account
      await this.asyncRequest('account_info', { account: address });

      const { result } = await this.asyncRequest('account_tx', { account: address, ...options });
      const { transactions: v4Transactions, ...otherInfo } = result;
      const transactions = v4Transactions.map(({ meta, tx_json, validated, hash }) => {
        const tx = {
          ...tx_json,
          Amount: meta.delivered_amount,
          inLedger: tx_json.ledger_index,
          hash
        };

        return { meta, validated, tx };
      });

      return { ...otherInfo, transactions };
    } catch (err) {
