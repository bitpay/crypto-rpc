...
class XrpRpc {
  ...
  async signTransaction({ address, amount, passphrase, tag, invoiceID, secret }) {
  ...
  async submitSignedTransaction({ signedTx }, alreadyConnected = false) {
    if (!alreadyConnected) {
      await this.rpc.connect();
    }
  ...
  async getBalance({ address }) {
    ...
    return balance.find((b) => (b.currency === this.config.currency)).value;
  }
