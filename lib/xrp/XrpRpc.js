...
class XrpRpc {
  ...
  async getBlock({ hash, index } = {}) {
    try {
      let result;
      if ([hash, index].includes('latest') || (!hash && !index)) {
        ({ result } = await this._getLatestBlock());
      } else if (hash) {
        ({ result } = await this._getBlockByHash({ hash }));
      } else {
        ({ result } = await this._getBlockByIndex({ index }));
      }
      return result;
    } catch (err) {
  ...
  _getLatestBlock() {
    return this.asyncRequest('ledger_closed');
  }

  _getBlockByHash({ hash }) {
