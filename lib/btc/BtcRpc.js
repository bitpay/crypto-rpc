...
class BtcRpc {
  ...
  async sendMany({ batch, options }) {
    ...
    return this.asyncCall('sendMany', array);
  }
  ...
  async unlockAndSendManyBatched({ batchArray, passphrase, options, time = 10800 }) {
    if (passphrase === undefined) {
      passphrase = await passwordPromptAsync('> ');
    }
    await this.walletUnlock({ passphrase, time: time });
    this.emitter.emit('unlocked', time);
    let txSuccessResults = [];
    let txFailureResults = [];
    let failureBool;
    for (let batch of batchArray) {
      ...
      try {
        const txid = await this.sendMany({ batch:batch, options:options });
        emitData.txid = txid;
        this.emitter.emit('success', emitData);
        txSuccessResults[txid] = batch;
      } catch(error) {
        emitData.error = error;
        this.emitter.emit('failure', emitData);
        txFailureResults.push(emitData);
        failureBool = true;
      }
    }
    ...
    if (failureBool) {
      throw {
        name: 'Batched Transaction Failure',
        message: 'At least one of many requests Failed',
        data: txFailureResults
      };
    }
    this.emitter.emit('done');
    return {txFailureResults, txSuccessResults};
  }
  ...
  async unlockAndSendToAddressMany({ payToArray, passphrase, time = 10800 }) {
