...
class BtcRpc {
  ...
  async unlockAndSendToAddressMany({ payToArray, passphrase, time=10800, maxValue=10*1e8, maxOutputs=1 }) {
    ...
    if (payToArray.length < maxOutputs) {
      maxOutputs = payToArray.length;
    }
    ...
    return [batchArray, txFailureResults, txSuccessResults];
  }
