...
class EthRPC {
  ...
  async sendToAddress({ address, amount, sendAccount, passphrase }) {
    const gasPrice = await this.estimateGasPrice();
    const account = sendAccount || await this.getAccount();
  ...
  async unlockAndSendToAddress({ address, amount, sendAccount, passphrase }) {
    if (passphrase === undefined) {
      passphrase = await passwordPromptAsync('> ');
    }
    console.warn('Unlocking for a single transaction.');
    const tx = await this.sendToAddress({ address, amount, sendAccount, passphrase });
    return tx;
  }

  async unlockAndSendToAddressMany({ payToArray, passphrase, sendAccount }) {
    if (passphrase === undefined) {
      passphrase = await passwordPromptAsync('> ');
    }
    ...
    for (const payment of payToArray) {
      ...
      try {
        const txid = await this.sendToAddress({ address, amount, sendAccount, passphrase });
        emitData.txid = txid;
