...
class EthRPC {
  ...
  async estimateGasPrice() {
    ...
    let percentile75GasPrices = [];
    for (const block of blocks) {
      if (block && block.transactions.length) {
        const gasPrices = block.transactions.map(tx => parseInt(tx.gasPrice)).sort((a, b) => b - a);
        const percentile75 = gasPrices[Math.floor(block.transactions.length / 4 * 3)];
        percentile75GasPrices.push(percentile75);
      }
    }
    const rpcEstimate = parseInt(await this.web3.eth.getGasPrice());
    if (!percentile75GasPrices.length) {
      return rpcEstimate;
    }
    const shortAverage = Math.ceil(percentile75GasPrices.slice(0, percentile75GasPrices.length / 2).reduce((acc, cur) => acc + cur, 0) / (percentile75GasPrices.length / 2));
    const longAverage = Math.ceil(percentile75GasPrices.reduce((acc, cur) => acc + cur, 0) / percentile75GasPrices.length);
  ...
  sendRawTransaction({ rawTx }) {
    return new Promise((resolve, reject) => {
      const errorHandler = (err) => {
        if (err && err.message && err.message.includes('already imported')) {
