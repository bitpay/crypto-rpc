...
class EthRPC {
    ...
    return new Promise((resolve, reject) => {
      const errorHandler = (err) => {
        if (err && err.message && (err.message.includes('already imported') || err.message.includes('already known'))) {
          ...
          const txid = ethers.utils.serializeTransaction({
            to,
            nonce,
            gasLimit,
            gasPrice,
  ...
  async decodeRawTransaction({ rawTx }) {
    ...
    return {
      ...
      nonce,
      txid: hash,
      value: value ? value.toNumber() : undefined,
      type: type || undefined,
      gasPrice: gasPrice ? gasPrice.toNumber(): undefined,
      maxPriorityFeePerGas: maxPriorityFeePerGas ? maxPriorityFeePerGas.toNumber() : undefined,
      maxFeePerGas: maxFeePerGas ? maxFeePerGas.toNumber() : undefined,
      gasLimit: gasLimit ? gasLimit.toNumber() : undefined,
      data
    };
  }
