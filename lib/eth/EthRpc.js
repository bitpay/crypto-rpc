...
class EthRPC {
  ...
  async unlockAndSendToAddress({ address, amount, fromAccount, passphrase, gasPrice, nonce }) {
    if (passphrase === undefined) {
      passphrase = await passwordPromptAsync('> ');
    }
    console.warn('Unlocking for a single transaction.');
    const tx = await this.sendToAddress({ address, amount, fromAccount, passphrase, gasPrice, nonce });
    return tx;
  }

  async unlockAndSendToAddressMany({ payToArray, passphrase, fromAccount, gasPrice, nonce }) {
    if (passphrase === undefined) {
      passphrase = await passwordPromptAsync('> ');
    }
    ...
    for (const payment of payToArray) {
      ...
      try {
        const txid = await this.sendToAddress({ address, amount, fromAccount, passphrase, gasPrice, nonce });
        emitData.txid = txid;
        ...
        await new Promise(async (resolve, reject) => {
          try {
            let confirmations = 0;
            let timeout = new Date().getTime() + 45000;
