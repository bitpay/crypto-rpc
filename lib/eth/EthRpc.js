...
class EthRPC {
  ...
  async sendToAddress({ address, amount, fromAccount, passphrase, gasPrice }) {
    if (!gasPrice) {
  ...
  async unlockAndSendToAddress({ address, amount, fromAccount, passphrase, gasPrice }) {
    if (passphrase === undefined) {
      passphrase = await passwordPromptAsync('> ');
    }
    console.warn('Unlocking for a single transaction.');
    const tx = await this.sendToAddress({ address, amount, fromAccount, passphrase, gasPrice });
    return tx;
  }

  async unlockAndSendToAddressMany({ payToArray, passphrase, fromAccount, gasPrice }) {
    if (passphrase === undefined) {
      passphrase = await passwordPromptAsync('> ');
    }
    ...
    for (const payment of payToArray) {
      ...
      try {
        const txid = await this.sendToAddress({ address, amount, fromAccount, passphrase, gasPrice });
        emitData.txid = txid;
