...
class SolRPC {
   ...
   * @param {number} [maxDepth] - Optional. Maximum nesting depth for recursive ATA discovery. If -1, treated as Infinity.
   * @returns 
   */
   ...
   * @param {number} [params.maxDepth=5] - Safety guard to limit recursion depth. Use Infinity for unlimited depth.
   * @returns 
   */
  async getTokenAccountsByOwner({ address, skipExistenceCheck = false, maxDepth = 5 }) {
    ...
    // Normalize maxDepth: -1 => Infinity; > 0 => keep; otherwise default to 5
    if (maxDepth === -1) {
      maxDepth = Infinity;
    } else if (!(typeof maxDepth === 'number' && maxDepth > 0)) {
      maxDepth = 5;
    ...
    const nextDepth = maxDepth === Infinity ? Infinity : Math.max(0, maxDepth - 1);
    ...
    for (const token of tokens) {
      ...
      const nestedAtas = nextDepth > 0
        ? await this.getTokenAccountsByOwner({ address: pubkey, skipExistenceCheck: true, maxDepth: nextDepth })
        : [];
