  ...
  async deriveAta({ solAddress, mintAddress }) {
    try {
      ...
      if (err.message.toLowerCase().includes('cannot read properties')) {
        ...
        if (!solAddress) missingParamsArray.push('solAddress');
        if (!mintAddress) missingParamsArray.push('mintAddress');
        throw new Error(`Missing parameters: ${missingParamsArray.join(', ')}`);
      }
      throw err;
    }
  }
