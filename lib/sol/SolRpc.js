  ...
  async sendToAddress({ 
    ...
    try {
      ...
      let transactionMessage = pipe(
        ...
        tx => SolKit.setTransactionMessageFeePayerSigner(feePayerKeypairSigner || fromAccountKeypairSigner, tx),
      );
 ...
 * @returns {{ messageBytes: import('@solana/kit').TransactionMessageBytes; signatures: import('@solana/kit').SignaturesMap } | null} The decoded transaction or null if the input is invalid.
 */
 ...
 */
  async getConfirmations({ txid }) {
    ...
    const { value: statuses } = await this.rpc.getSignatureStatuses([SolKit.signature(txid)]).send();
 ...
 * @returns {import("@solana/kit").Address<string>} The converted Web3 PublicKey instance.
 */
  toAddress(addressStr) {
    return SolKit.address(addressStr);
  }
 ...
 */
  ...
  /**
   * @param {import('@solana/kit').TransactionMessageBytes} messageBytes 
   * @returns {import('@solana/kit').TransactionMessageBytesBase64}
   */
...
* @property {import("@solana/kit").UnixTimestamp} blockTime
* @property {import("@solana/kit").Blockhash} blockhash
* @property {import("@solana/kit").Slot} parentSlot
* @property {import("@solana/kit").Blockhash} previousBlockhash
*/