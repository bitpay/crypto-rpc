...
class SolRPC {
  ...
  async decodeRawTransaction({ rawTx }) {
    ...
    if ('addressTableLookups' in compiledTransactionMessage &&
   ...
   * @param {number} [maxDepth] - Optional. Maximum nesting depth for recursive ATA discovery. If -1, treated as Infinity. Minimum value is 1.
   * @returns 
   */
   ...
   * @param {number} [params.maxDepth=5] - Maximum total calls allowed (1 = no recursion, 2 = one level of recursion, etc). Use Infinity for unlimited depth. Minimum value is 1.
   * @returns 
   */
  async getTokenAccountsByOwner({ address, skipExistenceCheck = false, maxDepth = 5 }) {
    ...
    // Normalize maxDepth: -1 => Infinity; >= 1 => keep; otherwise default to 5
    if (maxDepth === -1) {
      maxDepth = Infinity;
    } else if (!(typeof maxDepth === 'number' && maxDepth >= 1)) {
      maxDepth = 5;
    ...
    // Calculate remaining depth: maxDepth 1 = no recursion, maxDepth 2 = 1 recursion, etc.
    const remainingDepth = maxDepth === Infinity ? Infinity : maxDepth - 1;
    ...
    for (const token of tokens) {
      ...
      const nestedAtas = remainingDepth > 0
        ? await this.getTokenAccountsByOwner({ address: pubkey, skipExistenceCheck: true, maxDepth: remainingDepth })
        : [];

      atas.push({ mint, state, pubkey, atas: nestedAtas });
    }
