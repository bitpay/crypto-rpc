...
class SolRPC {
  ...
  async estimateTransactionFee({ rawTx }) {
    let tx;
    try {
      tx = await this.decodeRawTransaction({ rawTx });
  ...
  async getTransaction({ txid }) {
    ...
    const serializedTransaction = await this.getRawTransaction({ txid });

    const base64Encoder = SolKit.getBase64Encoder();
    const transactionBytes = base64Encoder.encode(serializedTransaction);

    return this.decodeRawTransaction({ rawTx: transactionBytes });
  }
