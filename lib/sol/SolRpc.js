...
class SolRPC {
  ...
  async getTransaction({ txid }) {
    ...
    const decodeRawTransactionParams = { 
      rawTx: transactionBytes,
      useAddressLookupTables: meta?.loadedAddresses !== undefined
    };
   ...
   * @param {boolean} params.useAddressLookupTables - Meta loaded addresses for lookup table transactions
   * @returns {Object} if txid is null, it indicates the transaction has not yet been signed by the fee payer
   */
  async decodeRawTransaction({ rawTx, useAddressLookupTables }) {
    if (typeof rawTx === 'string') {
      rawTx = Buffer.from(rawTx, 'base64');
    }
    ...
    if (useAddressLookupTables && 'addressTableLookups' in compiledTransactionMessage &&
