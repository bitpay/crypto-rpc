...
class SolRPC {
  ...
  async getTransaction({ txid }) {
    ...
    const { blockTime, transaction, meta } = serializedTransactionObject;
    // transaction is an array with [rawTx, 'base64']
    ...
    if (parsedTransaction.instructions.transferToken && Array.isArray(meta.preTokenBalances) && Array.isArray(meta.postTokenBalances)) {
      parsedTransaction.atas = this.#prepareTokenBalanceChange({ preTokenBalances: meta.preTokenBalances, postTokenBalances: meta.postTokenBalances });
    }
    return { ...parsedTransaction, blockTime: Number(blockTime) };
  }

  /**
   * 
   * @param {Object} params
   * @param {any[]} params.preTokenBalances
   * @param {any[]} params.postTokenBalances 
   */
  #prepareTokenBalanceChange({ preTokenBalances, postTokenBalances }) {
    const atas = preTokenBalances.reduce((acc, preTokenBalance) => {
      const { mint, owner } = preTokenBalance;
      /** @type {SolKit.TokenAmount} */
      const uiTokenPreAmount = preTokenBalance.uiTokenAmount;
      const correspondingPostTokenBalance = postTokenBalances.find(postTokenBalance => postTokenBalance.mint === mint && postTokenBalance.owner === owner);
      if (correspondingPostTokenBalance) {
        /** @type {SolKit.TokenAmount} */
        const uiTokenPostAmount = correspondingPostTokenBalance.uiTokenAmount;
        acc.push({ mint, owner, amount: uiTokenPostAmount.amount - uiTokenPreAmount.amount });
      }
      return acc;
    }, []);
    return atas;
  ...
  async decodeRawTransaction({ rawTx }) {
    ...
    if (!feePayerSignatureBytes) {
      throw new Error('Transaction signature could not be determined because fee payer signature was not found');
    }
    const base58Decoder = SolKit.getBase58Decoder();
    const txid = base58Decoder.decode(feePayerSignatureBytes);
  ...
  async createAta({ ownerAddress, mintAddress, feePayer }) {
    ...
    const signedTransactionMessage = await SolKit.signTransactionMessageWithSigners(transactionMessage);
    const sendAndConfirmNonceTransaction = SolKit.sendAndConfirmTransactionFactory({ rpc: this.rpc, rpcSubscriptions: this.rpcSubscriptions });
  ...
  async getAccountInfo({ address }) {
    try {
      ...
      const lamports = Number(accountInfoResponse.value.lamports);
      const atas = await this.getTokenAccountsByOwner({ address, skipExistenceCheck: true });
   ...
   * @returns 
   */
  async getTokenAccountsByOwner({ address, skipExistenceCheck = false }) {
    ...
    for (const token of tokens) {
      const { mint, state } = token.account.data.parsed.info;
      atas.push({ mint, state, pubkey: token.pubkey });
    }
