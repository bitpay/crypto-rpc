...
class SolRPC {
  ...
  /**
   * Derives the ATA account address associated with the provided solAddress for the provided mint - does not create account
   * @param {Object} params
   * @param {string} params.solAddress
   * @param {string} params.mintAddress 
   */
  async getAta({ solAddress, mintAddress }) {
    try {
   ...
   * @param {string} params.mintAddress
   * @param {SolKit.KeyPairSigner} [params.feePayer] If not provided, owner pays fees
   */
  async createAta({ ownerAddress, mintAddress, feePayer }) {
    let ataAddress;
    try {
      ataAddress = await this.getAta({ solAddress: ownerAddress, mintAddress });
      if (ataAddress) {
        return {
          action:  'RETRIEVED',
          ataAddress,
