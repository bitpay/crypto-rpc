...
class SolRPC {
  ...
  async getTransaction({ txid }) {
    ...
    if (meta?.loadedAddresses !== undefined) {
      decodeRawTransactionParams.lookupTableAddresses = meta.loadedAddresses;
    }
    const parsedTransaction = await this.decodeRawTransaction(decodeRawTransactionParams);
    return {
      ...parsedTransaction,
   ...
   * @param {import('@solana/kit').TransactionMetaLoadedAddresses} params.lookupTableAddresses - Meta loaded addresses for lookup table transactions
   * @returns {Object} if txid is null, it indicates the transaction has not yet been signed by the fee payer
   */
  async decodeRawTransaction({ rawTx, lookupTableAddresses }) {
    if (typeof rawTx === 'string') {
      rawTx = Buffer.from(rawTx, 'base64');
    }
    ...
    if (lookupTableAddresses && 'addressTableLookups' in compiledTransactionMessage &&
      compiledTransactionMessage.addressTableLookups !== undefined) {
      decompileTransactionMessageConfig.addressesByLookupTableAddress = this.createAddressesByLookupTableAddressFromLoadedAddresses({
        loadedAddresses: lookupTableAddresses,
        addressTableLookups: compiledTransactionMessage.addressTableLookups,
      });
    }
    const decompiledTransactionMessage = SolKit.decompileTransactionMessage(compiledTransactionMessage, decompileTransactionMessageConfig);
  ...
  /**
   * Generates an map of addresses by lookup table address to be used as a config for decompileTransactionMessage
   */
  ...
  createAddressesByLookupTableAddressFromLoadedAddresses({
    ...
    for (const lookup of addressTableLookups) {
      ...
      for (const readableIndex of lookup.readonlyIndexes) {
  ...
  async sendRawTransaction({ rawTx }) {
    try {
      ...
      if (SolKit.isSolanaError(err)) {
        if (err.context.__code === -32002) {
          throw new Error(SOL_ERROR_MESSAGES.BLOCKHASH_NOT_FOUND_FOR_SEND);
        }
      }
      throw err;
    }
