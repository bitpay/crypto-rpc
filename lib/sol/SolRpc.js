 ...
 */
  constructor(config) {
    this.config = config;
    this.rpcUrl = `${this.config.protocol}://${this.config.host}`;
    this.rpcSubscriptionUrl = this.rpcUrl.replace('http', 'ws');
 ...
 */
  ...
  initRpcSubscriptions() {
    return createSolanaRpcSubscriptions(this.rpcSubscriptionUrl);
  }
  ...
  async cleanup() {
    ...
    if (this.rpcSubscriptions) {
      ...
      if (typeof subscriptionEvents === 'object') {
        Object.keys(subscriptionEvents).forEach(event => {
 ...
 */
  async getBalance({ address }) {
    ...
    return this.#bigIntToNumber(lamports);
  }
 ...
 */
  async sendToAddress({ 
    ...
    try {
      ...
      transactionMessage = await this.#setTransactionMessageLifetime({ transactionMessage, nonceAddressStr, nonceAuthorityAddressStr: (feePayerKeypairSigner || fromAccountKeypairSigner).address });
      ...
      signedTransactionMessage = await signTransactionMessageWithSigners(transactionMessage);
      ...
      if (err.message.includes('is no longer valid. It has advanced to')) {
        return signedTransactionMessage ? getSignatureFromTransaction(signedTransactionMessage) : null;
      }
 ...
 */
  async #setTransactionMessageLifetime({ nonceAddressStr, nonceAuthorityAddressStr, transactionMessage }) {
    ...
    if (nonceAddressStr) {
      ...
      transactionMessageWithLifetime = setTransactionMessageLifetimeUsingBlockhash(blockhashLifetimeConstraint, transactionMessage);
    }
 ...
 */
  async estimateFee({ rawTx, nBlocks = 10 }) {
    ...
    for (const sample of samples) {
      const slot = this.#bigIntToNumber(sample.slot);
 ...
 */
  async estimateTransactionFee({ rawTx }) {
    ...
    const base64EncodedMessage = this.#transactionMessageBytesToTransactionMessageBytesBase64(tx.messageBytes);
    ...
    return this.#bigIntToNumber(value);
  }
 ...
 */
  async estimateMaxPriorityFee({ config, percentile = 25 }) {
    ...
    const priorityFees = recentFees
      .filter(recentFee => this.#bigIntToNumber(recentFee.prioritizationFee) > 0)
      .map(recentFee => this.#bigIntToNumber(recentFee.prioritizationFee));
    ...
    return this.#bigIntToNumber(maxPriorityFeeBigInt);
  }
  ...
  async addPriorityFee({ transactionMessage, minPriorityFee, config }) {
    try {
      ...
      const transactionWithPriorityFee = appendTransactionMessageInstruction(
        getSetComputeUnitPriceInstruction({ microLamports: minBoundedPriorityFee }),
        transactionMessage
      );
 ...
 */
  async getRawTransaction({ txid }) {
    ...
    const tx = await this.rpc.getTransaction(signature(txid), { ...this._versionedConfig, encoding: 'base64' }).send();
    if (!tx?.transaction) {
      return null;
    }
 ...
 */
  async getBlock({ hash, height }) {
    if (typeof height === 'bigint') {
      height = this.#bigIntToNumber(height);
    }
 ...
 */
  async getTip() {
    ...
    const height = this.#bigIntToNumber(slot);
 ...
 */
   ...
   * @returns {import('@solana/web3.js').TransactionMessageBytesBase64}
   */
  #transactionMessageBytesToTransactionMessageBytesBase64(messageBytes) {
  ...
  /**
   * 
   * @param {import('@solana/web3.js').KeyPairSigner<string>} fromKeypair 
   * @param {import('@solana/web3.js').KeyPairSigner<string>} toKeypair 
  ...
  async #createRawTransaction(fromKeypair, toKeypair, amountInLamports) {
    ...
    const signedTransaction = await signTransactionMessageWithSigners(unsignedTransaction);
