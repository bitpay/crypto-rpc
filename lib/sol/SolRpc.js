...
class SolRPC {
  ...
  async getTransaction({ txid }) {
    ...
    const { blockTime, transaction } = serializedTransactionObject;
  ...
  async getAccountInfo({ address }) {
    try {
      ...
      if (SolKit.isSolanaError(err) && errMsg.includes('json-rpc') && errMsg.includes('should be less than 128 bytes')) {
        ...
        throw new Error('SolanaError: Request object exceeds 127 bytes. This may be caused by the provided address belonging to an Associated Token Account instead of a Solana account.');
      }
      throw err;
    }
  }
