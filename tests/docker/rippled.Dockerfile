...
RUN echo "deb https://repos.ripple.com/repos/rippled-deb focal stable" | sudo tee -a /etc/apt/sources.list.d/ripple.list
RUN sudo apt -y update
RUN sudo apt -y install rippled

RUN sudo rm /etc/opt/ripple/rippled.cfg
COPY tests/docker/rippled.cfg /home/docker
