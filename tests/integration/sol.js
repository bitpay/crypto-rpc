...
describe('SolRpc Integration tests', () => {
    ...
    before(async () => {
        ...
        // );
        const [keypairSigner1Value, keypairSigner2Value] = Object.values(config.currencyConfig).map(key => Keypair.fromSecretKey(bs58.decode(key)))

        // For these tests, the nonce authority will be the sender
        senderKeypair = keypairSigner1Value;
        nonceAuthorityKeypair = keypairSigner1Value;
    ...
    describe('getBalance', () => {
        it('can retrieve a balance number for a valid address', async () => {
            const addressString = senderKeypair.publicKey.toBase58();
    ...
    describe('estimateFee', () => {
        ...
        // Broken - getFeeCalculatorForBlockhash deprecated (and removed)
        // it('returns a number based on the average fee calculator for the last n blocks', async () => {
        //     const retVal = await solRpc.estimateFee({});
        //     console.log(retVal);
        // })
        // Code doesn't behave as expected - Web3.VersionedTransaction.deserialize throws
